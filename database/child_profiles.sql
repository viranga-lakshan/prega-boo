-- Supabase schema: child profiles (added by midwife, visible to mom and eligible midwife)
-- Run in Supabase SQL Editor.

create extension if not exists "pgcrypto";

-- Requires mom_profiles, user_roles, midwife_profiles to exist.
...
create table if not exists public.child_profiles (
  ...
  -- Optional extended registration details
  gender text,
...
-- If table already exists, safely add new columns
...
-- Mom can read their own children
...
-- Midwife can read children only for moms in same district
...
-- Midwife can add children only for moms in same district
