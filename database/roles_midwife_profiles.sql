-- Supabase schema: roles + midwife profiles (+ midwife read access to moms list)
-- Run in Supabase SQL Editor.

-- Optional (usually already enabled on Supabase)
create extension if not exists "pgcrypto";

-- Shared helper used by both mom_profiles and midwife_profiles
...
-- Create an enum type for app roles (idempotent)
do $$
begin
...
-- User roles table (links auth.users to an app role)
...
-- A signed-in user can read their own role
...
-- A signed-in user can create their own role row (client-side role assignment)
...
-- Auto-update updated_at for midwife_profiles
-- (Safe even if the trigger already exists)
...
-- Midwives can read/write only their own profile row
...
-- Allow MIDWIFE role to view all moms (for the Moms List screen)
-- NOTE: mom_profiles must already exist; if not, run database/mom_profiles.sql first.
